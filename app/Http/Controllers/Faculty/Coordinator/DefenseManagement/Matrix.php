    ...
    public function index()
    {
        ...
        $defenseMatrices = DB::table('tbl_defense_matrices as dm')
            ...
            ->select(
                ...
                DB::raw("CONCAT((3 + (ays.year - sy.year)), sa.section, LPAD(s.id, 3, '0')) AS group_code"),                
                DB::raw("(3 + (ays.year - sy.year)) AS year_level"),
                'sa.section',
                ...
                'cp.confirmed_panels',
                DB::raw("CONCAT(COALESCE(s.first_name,''), ' ', COALESCE(s.last_name,''), IF(s.suffix IS NOT NULL AND s.suffix <> '', CONCAT(' ', s.suffix), '')) AS proponents"),
        ...
        $availableProjects = DB::table('tbl_endorsements as e')
            ...
            ->select(
                ...
                't.title as project_title',
                DB::raw("CONCAT((3 + ($activeYear - sy.year)), sa.section, LPAD(s.id, 3, '0')) as group_code")
            )
            ->get();
